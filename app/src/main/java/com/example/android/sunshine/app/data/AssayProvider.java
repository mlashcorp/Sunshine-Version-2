...
public class AssayProvider extends ContentProvider {
    ...
    /*
        Students: We've coded this for you.  We just create a new AssayDbHelper for later use
        here.
     */
    @Override
    public boolean onCreate() {
        mOpenHelper = new AssayDbHelper(getContext());
        return true;
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case WEATHER: {
                ...
                if ( _id > 0 )
                    returnUri = AssayContract.AssayEntry.buildAssayUri(_id);
                else
    ...
    private void normalizeDate(ContentValues values) {
        ...
        if (values.containsKey(AssayContract.AssayEntry.COLUMN_DATE)) {
            long dateValue = values.getAsLong(AssayContract.AssayEntry.COLUMN_DATE);
            values.put(AssayContract.AssayEntry.COLUMN_DATE, AssayContract.normalizeDate(dateValue));
        }
    }
            ...
            case WEATHER:
                ...
                try {
                    for (ContentValues value : values) {
                        normalizeDate(value);
                        long _id = db.insert(AssayContract.AssayEntry.TABLE_NAME, null, value);
                        if (_id != -1) {
                            returnCount++;
                        }
                    }
